...
import  TRACE ; TRACE_NAME = "grimm" ;
#------------------------------------------------------------------------------
class GrimmSubmitJob:
    ...
    def ParseParameters(self):
        ...
        try:
            optlist, args = getopt.getopt(sys.argv[1:], '',
                     ['project=', 'verbose=', 'job=', 'doit=', 'dsid=', 'fileset=', 'recover=', 'stage=', "use_odb=" ] )
...
#------------------------------------------------------------------------------
            ...
            self.client    = midas.client.MidasClient("grimm_submit_job",self.host,self.expt_name,None)
            
        TRACE.INFO(f'-- END: fUseODB:{self.fUseODB}')
        return 0
...
#------------------------------------------------------------------------------
    def save_job_status(self):
        ...
        if (not os.path.exists(job_status_dir)): 
            ...
            rj              = json.dumps(r)
            # and save the dictionary in .json format
            if (self.fUseODB):
                odb_path = '/Mu2e/Offline/grim_projects/'+self.fProject+'/grid_jobs/'+jobid;
                self.client.set_odb(odb_path,r);
                TRACE.DEBUG(0,f'saved record in ODB')
            else:
                f               = open(status_fn,'w')
                f.write(rj)
                f.close()
...
#------------------------------------------------------------------------------
# main program, just make a GrimmSubmitJob instance and call its methods
#------------------------------------------------------------------------------
